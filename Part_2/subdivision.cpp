...
for(int i=0; i<S.size()-1 ;++i){
	while( conditions(marking[i]) == false && S[i+1].size()>m/2){  // S[i+1].size>m/2: we put a threshold for points transfered   
	...
	if (conditions(marking[i])==false) {std::cout<<i<<"Error, can't find valid q point\n"; exit(1);}  
